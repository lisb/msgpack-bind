...
public class SubclassSourceMarshaller extends ObjectMarshaller<SubclassSource> {
    ...
    private final Marshaller<Long> subFirstMarshaller = LongMarshaller.getInstance();

    protected int getSize(SubclassSource target) {
        int mapSize = 2;
    ...
    protected void marshalContent(MessagePacker packer, SubclassSource target) throws IOException {
        if (target.first != null) {
        ...
        subFirstMarshaller.marshal(packer, target.subFirst);
    }
}